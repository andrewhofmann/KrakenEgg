...
const UltraFileList = ({
  ...
  const scrollToFocusedFile = useCallback((focusedFileId?: string, callerSource?: string) => {
    ...
    console.log(`🚀 scrollToFocusedFile called by: ${callerSource || 'useEffect'}, isActive: ${isActive}, focusedFile: ${targetFocusedFile}`);

    if (isActive && targetFocusedFile && containerRef.current) {
      const index = sortedFiles.findIndex(f => f.id === targetFocusedFile);
      console.log(`📊 SCROLL DEBUG: focusedFile: ${targetFocusedFile}, index: ${index}, sortedFiles.length: ${sortedFiles.length}`);
      ...
      if (index !== -1) {
        ...
        console.log(`📊 SCROLL CALC: index=${index}, itemHeight=${itemHeight}, targetScrollTop=${targetScrollTop}`);
        console.log(`📊 CONTAINER: height=${containerHeight}, currentScrollTop=${currentScrollTop}`);
        ...
        console.log(`📊 VISIBILITY: itemTop=${itemTop}, itemBottom=${itemBottom}, visibleTop=${visibleTop}, visibleBottom=${visibleBottom}`);
        ...
        if (itemTop < visibleTop || itemBottom > visibleBottom) {
          ...
          if (itemTop < visibleTop) {
            ...
            console.log(`⬆️ SCROLLING UP: item above visible area, newScrollTop=${newScrollTop}`);
          } else {
            // Item is below visible area - scroll down to show it at bottom
            newScrollTop = itemBottom - containerHeight;
            console.log(`⬇️ SCROLLING DOWN: item below visible area, newScrollTop=${newScrollTop}`);
          }
          ...
          console.log(`✅ FINAL SCROLL: ${currentScrollTop} -> ${newScrollTop}, maxScroll=${maxScroll}`);
          container.scrollTop = newScrollTop;
          console.log(`🎯 Scrolled to focused file "${sortedFiles[index]?.name}" at index ${index}`);
        } else {
          console.log(`👀 Item already visible, no scroll needed`);
        }
      } else {
        console.log(`❌ Focused file not found in sorted files`);
      }
    } else {
      console.log(`❌ Cannot scroll: isActive=${isActive}, focusedFile=${targetFocusedFile}, container=${!!containerRef.current}`);
    }
  }, [isActive, panel.focusedFile, sortedFiles, itemHeight]);
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    console.log(`⌨️ Key pressed: ${event.key}, isActive: ${isActive}, fileCount: ${sortedFiles.length}`);

    if (!isActive || sortedFiles.length === 0) {
      console.log(`❌ Ignoring key press - isActive: ${isActive}, fileCount: ${sortedFiles.length}`);
      return;
    }
    ...
    console.log(`📍 Current index: ${currentIndex}, focused file: ${panel.focusedFile}`);
    ...
    keyNavigationThrottle.current = requestAnimationFrame(() => {
      ...
      // Use unified scroll function for consistency
      if (newFocusedFile) {
        console.log(`⌨️ KEYBOARD NAV: Moving to file "${newFocusedFile.name}" at index ${newIndex}`);
        // Use small timeout to ensure state update is processed first
        setTimeout(() => {
          scrollToFocusedFile(newFocusedFile.id, 'keyboard');
        }, 0);
      }
    });
  }, [isActive, sortedFiles, panel.focusedFile, onUpdatePanel, handleFileDoubleClick, itemHeight, scrollToFocusedFile]);
