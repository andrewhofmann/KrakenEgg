...
export const useKeyboardNavigation = (
  ...
  const pendingNavigationUpdate = useRef<number | null>(null);

  // Cache sorted files to avoid expensive recalculation on every keystroke
  const sortedFilesCache = useRef<{
    path: string;
    sortBy: string;
    sortOrder: string;
    showHidden: boolean;
    panelId: string;
    sortedFiles: any[];
    fileIndexMap: Map<string, number>; // Fast O(1) lookup for focused file index
    prevIndexMap: Map<string, number>; // Pre-computed previous indices for up navigation
    nextIndexMap: Map<string, number>; // Pre-computed next indices for down navigation
  } | null>(null);
  ...
  const handleKeyboardAction = useCallback((action: string) => {
    ...
    // Get cached sorted files for maximum performance
    const getCachedSortedFiles = () => {
      const cacheKey = `${activePanel.currentPath}-${activePanel.sortBy}-${activePanel.sortOrder}-${activePanel.showHidden}-${appState.activePanel}`;

      if (sortedFilesCache.current &&
          sortedFilesCache.current.path === activePanel.currentPath &&
          sortedFilesCache.current.sortBy === activePanel.sortBy &&
          sortedFilesCache.current.sortOrder === activePanel.sortOrder &&
          sortedFilesCache.current.showHidden === activePanel.showHidden &&
          sortedFilesCache.current.panelId === appState.activePanel) {
        return sortedFilesCache.current.sortedFiles;
      }

      // Cache miss - calculate and store
      const sortedFiles = sortFiles(
        filterFiles(activePanel.files, '', activePanel.showHidden),
        activePanel.sortBy,
        activePanel.sortOrder
      );

      // Build O(1) lookup maps for file indices and pre-computed navigation
      ...
      sortedFiles.forEach((file, index) => {
        ...
        prevIndexMap.set(file.id, Math.max(0, index - 1));
        nextIndexMap.set(file.id, Math.min(sortedFiles.length - 1, index + 1));
      });

      sortedFilesCache.current = {
        path: activePanel.currentPath,
        sortBy: activePanel.sortBy,
        sortOrder: activePanel.sortOrder,
        showHidden: activePanel.showHidden,
        panelId: appState.activePanel,
        sortedFiles,
        fileIndexMap,
        prevIndexMap,
        nextIndexMap
    ...
    // Only calculate sorted files for navigation actions that need them
    let sortedFiles: any[] = [];
    let focusedFileIndex = -1;
    let focusedFile = null;

    if (action === 'navigateUp' || action === 'navigateDown' || action.includes('Selection') || action === 'enterDirectory' || action === 'toggleSelection' || action === 'renameFile' || action === 'view' || action === 'edit') {
      sortedFiles = getCachedSortedFiles();

      if (activePanel.focusedFile && sortedFilesCache.current?.fileIndexMap) {
        // Use O(1) Map lookup instead of O(n) findIndex
        const foundIndex = sortedFilesCache.current.fileIndexMap.get(activePanel.focusedFile);
        focusedFileIndex = foundIndex !== undefined ? foundIndex : -1;
        focusedFile = focusedFileIndex >= 0 ? sortedFiles[focusedFileIndex] : null;
      }
    }
    ...
    switch (action) {
      ...
      // File navigation (instant DOM updates with pre-computed indices)
      case 'navigateUp':
        if (sortedFiles.length > 0 && activePanel.focusedFile && sortedFilesCache.current?.prevIndexMap) {
          const prevIndex = sortedFilesCache.current.prevIndexMap.get(activePanel.focusedFile);
          if (prevIndex !== undefined) {
            flushSync(() => {
              updateActivePanel(panel => ({
                ...panel,
                focusedFile: sortedFiles[prevIndex]?.id || null
              }));
            });
          }
        }
      ...
      case 'navigateDown':
        if (sortedFiles.length > 0 && activePanel.focusedFile && sortedFilesCache.current?.nextIndexMap) {
          const nextIndex = sortedFilesCache.current.nextIndexMap.get(activePanel.focusedFile);
          if (nextIndex !== undefined) {
            flushSync(() => {
              updateActivePanel(panel => ({
                ...panel,
                focusedFile: sortedFiles[nextIndex]?.id || null
              }));
            });
          }
        }
      ...
      case 'extendSelectionDown':
        if (sortedFiles.length > 0 && focusedFileIndex >= 0) {
          const newIndex = Math.min(sortedFiles.length - 1, focusedFileIndex + 1);
          ...
          if (targetFile && targetFile.id !== activePanel.focusedFile) {
            updateActivePanel(panel => {
              ...
              const anchorIndex = anchor ? sortedFiles.findIndex(f => f.id === anchor) : focusedFileIndex;
      ...
      case 'extendSelectionUp':
        if (sortedFiles.length > 0 && focusedFileIndex >= 0) {
          const newIndex = Math.max(0, focusedFileIndex - 1);
          const targetFile = sortedFiles[newIndex];
