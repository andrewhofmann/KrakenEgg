            ...
            {info && !loading && (
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-2">
                        <div className="text-macos-textSecondary text-right pr-4">Total Size:</div>
                        <div className="font-semibold">{formatSize(info.size)}</div>
                        
                        <div className="text-macos-textSecondary text-right pr-4">Files:</div>
                        <div>{info.files.toLocaleString()}</div>
                        
                        <div className="text-macos-textSecondary text-right pr-4">Folders:</div>
                        <div>{info.folders.toLocaleString()}</div>
                    </div>
                    
                    <div className="pt-4 border-t border-white/10">
                        <div className="text-xs text-macos-textSecondary mb-1">Full Path</div>
                        <div className="break-all bg-black/20 p-2 rounded text-xs select-text">
                            {targetTab.path === "/" ? `/${currentItem}` : `${targetTab.path}/${currentItem}`}
                        </div>
                    </div>
                </div>
            )}
