        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm font-sans">
            <div className="w-full max-w-4xl bg-[#1e1e1e] border border-white/10 rounded-lg shadow-2xl flex flex-col max-h-[80vh] text-sm text-white">
                ...
                <div className="flex justify-between items-center p-4 border-b border-white/10 bg-white/5">
                    ...
                    <button onClick={closeMultiRename} className="p-1 hover:bg-white/10 rounded" aria-label="Close"><X size={18} /></button>
                </div>
                ...
                <div className="p-4 space-y-4 border-b border-white/10 bg-black/20">
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-2 border-t border-white/5">
                        <div>
                            ...
                            <input type="number" value={counterStart} onChange={e => setCounterStart(parseInt(e.target.value) || 1)} className="w-full bg-black/30 border border-white/10 rounded px-2 py-1" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Step by</label>
                            <input type="number" value={counterStep} onChange={e => setCounterStep(parseInt(e.target.value) || 1)} className="w-full bg-black/30 border border-white/10 rounded px-2 py-1" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Digits</label>
                            <input type="number" value={counterWidth} onChange={e => setCounterWidth(parseInt(e.target.value) || 1)} className="w-full bg-black/30 border border-white/10 rounded px-2 py-1" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-white/5">
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Regex Find</label>
                            <input
                                value={regexFind}
                                onChange={e => setRegexFind(e.target.value)}
                                placeholder="e.g. (\d{4})_(\d{2})"
                                className="w-full bg-black/30 border border-white/10 rounded px-2 py-1 font-mono text-xs placeholder:text-white/20 focus:border-blue-500 outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Regex Replace</label>
                            <input
                                value={regexReplace}
                                onChange={e => setRegexReplace(e.target.value)}
                                placeholder="e.g. $1-$2"
                                disabled={!regexFind}
                                className="w-full bg-black/30 border border-white/10 rounded px-2 py-1 font-mono text-xs placeholder:text-white/20 focus:border-blue-500 outline-none disabled:opacity-30"
                            />
                        </div>
                        <div>
                            ...
                            <select
                                ...
                                className="w-full bg-black/30 border border-white/10 rounded px-2 py-1.5 text-xs focus:border-blue-500 outline-none"
                            >
                            ...
                            {previews.map((p, i) => (
                                <tr key={i} className={clsx("border-b border-white/5 transition-colors", p.status === 'error' ? "bg-red-900/20 hover:bg-red-900/30" : "hover:bg-white/5")}>
                                    ...
                                    <td className={clsx("p-2 truncate max-w-xs font-mono", p.status === 'error' ? "text-red-400" : p.status === 'ok' && p.original !== p.new ? "text-green-400" : "text-gray-500")}>
                                        {p.new} {p.error && <span className="text-red-400 text-xs ml-2">{p.error}</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                {/* Footer */}
                <div className="p-4 border-t border-white/10 flex items-center gap-3 bg-[#2a2a2a]">
                    {error && <span className="text-red-400 mr-auto text-sm truncate">{error}</span>}
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed rounded font-bold text-sm flex items-center shadow-lg transition-all transform active:scale-95"
                    >
